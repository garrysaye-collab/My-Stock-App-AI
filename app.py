...
def get_data_with_indicators(stock_id):
    ...
    try:
        ...
        df['MA250'] = df['Close'].rolling(250).mean() # 加入年線
...
def chat_with_gemini(api_key, prompt_text, system_instruction):
    ...
    try:
        ...
        # ✅ 修正點：使用符合官方 SDK 標準的工具宣告格式
        # 避免將 google_search 誤判為 FunctionDeclaration
        tools_config = [
            {
        ...
        # 🚨 備用方案：如果 2.0 連線失敗或工具報錯，降級使用 1.5-flash
        try:
            ...
            response = chat_backup.send_message(prompt_text + "\n(系統提示：由於搜尋模組連線異常，此回應基於內建知識庫)")
            return response.text
        except Exception as e2:
            return f"❌ AI 連線嚴重錯誤: {str(e2)}"
    ...
    if st.button("📊 完整分析 (含聯網查證)", type="primary", use_container_width=True):
        ...
        with st.spinner("🔄 正在執行量化回測並召集大師團隊..."):
            ...
            if df is not None:
                ...
                first_msg = f"""
                【系統傳入數據 - {real_id}】
                1. 最新收盤: {latest['Close']:.2f} (MA20: {latest['MA20']:.2f}, MA60: {latest['MA60']:.2f}, MA250: {latest['MA250']:.2f})
