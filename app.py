...
# ==========================================
# 🧠 AI 分析模組 (已修正為最新 Flash 模型)
# ==========================================
def ask_gemini(api_key, stock_id, df, score, reasons):
    if not api_key:
        return None  # 如果沒有 Key，就不執行 AI
    
    try:
        # 設定 API
        genai.configure(api_key=api_key)
        
        # 🔥 使用最新的 Flash 模型 (速度快、免費額度高)
...
def get_data_and_analyze(stock_id):
    ...
    try:
        ...
        # 處理 MultiIndex 欄位問題 (yfinance 新版修正)
...
st.caption("結合量化數據與 Gemini AI 的智慧分析")
...
if run_btn:
    with st.spinner(f"正在連線交易所抓取 {ticker} 資料..."):
        ...
        if df is None:
            ...
            if api_key:
                with st.spinner("🤖 AI 正在閱讀財報與技術線圖，請稍候..."):
                    ai_response = ask_gemini(api_key, real_id, df, score, reasons)
                    if ai_response:
                        st.success("分析完成！")
                        st.markdown(ai_response)
                    else:
                        st.error("AI 無法回應，請檢查 API Key 是否正確。")
            else:
                st.info("💡 如果您輸入 Google API Key，這裡會顯示 AI 對這檔股票的具體操作建議。")

            # --- 3. 走勢圖 ---
            st.divider()
            st.subheader("📈 近期走勢圖 (100天)")
